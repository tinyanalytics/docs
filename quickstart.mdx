...
<AccordionGroup>
  <Accordion title="Do I need a cookie banner for tinyanalytics?">
    No. tinyanalytics is cookieless — it writes nothing to your visitor's device, so there is no storage to ask consent for. Visitors are counted with a short, one-way hash the server derives from the request instead. See [how tinyanalytics works](/how-it-works) for the mechanism.
  </Accordion>
  <Accordion title="Where do I put the tracking script in a framework or CMS?">
    Add it once in a shared layout, template, or header so it loads on every page. tinyanalytics has step-by-step guides for Next.js, React, WordPress, Webflow, Shopify, and more — see the [integration guides](/integrations/overview).
  </Accordion>
  <Accordion title="Does the tracking script work with single-page apps?">
    Yes, with no extra setup. Route tracking is on by default, so client-side navigation in React, Vue, and similar frameworks is recorded as a new pageview automatically. See [track single-page apps](/guides/script-configuration#track-single-page-apps) to change this.
  </Accordion>
