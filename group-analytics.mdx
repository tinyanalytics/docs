...
description: "Roll your metrics up by account — company, workspace, or team — instead of by visitor. Tag events with an account for account-grain funnels, retention, and counts."
icon: "building"
---
...
- Pass **traits** to describe the account — its name, plan, or size — the same way [user traits](/identify-users) describe a person. Update them later with `setGroupTraits(type, traits)`.
- Call `resetGroups()` to forget the associations, for example when a user logs out.

<Note>
  `group()` persists the associations you provide in browser storage so later events keep their account context. Call `resetGroups()` at logout, and review [Privacy & GDPR](/resources/privacy) when assessing consent requirements.
</Note>
...
[Funnels](/funnels) and [retention](/retention) can measure the **account** instead of the session or person. This changes what "completed" and "returned" mean:
...
- **Inactivity alerts** — get notified when an account that used to be active goes quiet, so you can reach out before it churns. See [alerts](/alerts).
- **Flag targeting by account** — evaluate a [feature flag](/feature-flags) per account so every seat in one company shares the same rollout, and rollout percentages count accounts rather than people.
