...
- an **autocaptured interaction** — an outbound link, button click, form submission, or copy (see [autocapture](/autocapture-analytics-events)).
...
For a custom-event step, start typing to filter event names recorded in the dashboard's current date range. The suggestions help you reuse the exact name, but you can still type a new event that has no data in that range.
...
Each recorded event can satisfy only one funnel step. When two different events happen in the same millisecond, TinyAnalytics gives them a stable order so they can advance consecutive steps, but a later step never walks backward and reuses an earlier event.
...
<AccordionGroup>
  <Accordion title="What can I use as a funnel step in TinyAnalytics?">
    A funnel step is a page (matched by its path), a custom event (matched by its name), or an [autocaptured](/autocapture-analytics-events) interaction — an outbound link, button click, form submission, or copy. You add steps in the order visitors should follow them, name the funnel, and save it. You can create as many funnels as you need for different flows.
  </Accordion>
  <Accordion title="Can a funnel step be a button click or outbound link?">
    Yes. A step can be an [autocapture](/autocapture-analytics-events) interaction — an outbound link, button click, form submission, or copy — with no custom event to write. Give the step a value pattern (with `*` and `**` wildcards) to match a specific interaction, or enter `*` to match any event of that type.
  </Accordion>
