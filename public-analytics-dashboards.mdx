...
sidebarTitle: "Public dashboards"
description: "Share a read-only analytics dashboard without a login through an open public link or a private bearer link that includes a secret key."
icon: "share-nodes"
---
...
Shared dashboards also never show **who** your visitors are. On a public or share-link view, the reports that can carry identified-person data — Users, Sessions, and Events — render without the name, email, username, or any custom trait you attached with [`identify()`](/user-identification-analytics). Viewers see the same anonymous rows an unidentified visitor produces. Only signed-in members of the site's organization (or an API key scoped `users:read`) see identified people.
...
<Columns cols={2}>
  <Card title="Embed a dashboard" icon="window-maximize" href="/embed-analytics-dashboard">
