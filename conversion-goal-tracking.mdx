...
- a **custom event** — a conversion happens when a visitor fires a matching [event](/custom-event-tracking), or
- an **autocaptured interaction** — a conversion happens when a visitor triggers a matching [autocapture](/autocapture-analytics-events) event (an outbound link, button click, form submission, or copy).
...
For a custom-event goal, the **Event name** field searches up to 100 custom events seen in the last 30 days and shows the count for each one. You can still type an event name or matching pattern that has not appeared yet. The amber “not seen” message is informational and does not block the goal.

Give the goal a name, and it starts tracking conversions.

## Can I turn an autocaptured interaction into a goal?

Yes. You can build a goal on any of four [autocapture](/autocapture-analytics-events) event types without writing a custom event:
...
<Note>
  **`input_change` isn't a goal target.** Individual field changes are captured for the [Events explorer](/event-analytics) but are too high-volume to build a conversion on, so they can't be used as a goal. Autocapture goals also can't aggregate [revenue](/revenue-analytics) — attach revenue to a [custom event](/custom-event-tracking) instead.
</Note>
...
- Use a [**funnel**](/funnel-analytics) to track a sequence of steps — "where did they drop off on the way?"

## Frequently asked questions

<AccordionGroup>
  <Accordion title="Can I measure a goal for one country or traffic source?">
    Yes. Goals respect the dashboard's date range and filters, so you measure the same goal for different sources, countries, or time periods by changing the filters — no second goal needed. See [Filter your data](/analytics-filters).
  </Accordion>
  ...
  <Accordion title="Can I set a goal on a button click or outbound link without writing code?">
    Yes. A goal can target an [autocapture](/autocapture-analytics-events) event — an outbound link, button click, form submission, or copy — with no custom event to write. Add an optional value pattern (with `*` and `**` wildcards) to narrow it, or leave the pattern blank to count every event of that type. Individual `input_change` events can't be used as a goal.
  </Accordion>
  <Accordion title="What's the difference between a goal and a funnel?">
    A goal tracks a single outcome — "did they convert?" — while a [funnel](/funnel-analytics) tracks an ordered sequence of steps and shows where people drop off on the way. Use a goal when you only need the conversion count and rate for one page view or custom event.
  </Accordion>
</AccordionGroup>
