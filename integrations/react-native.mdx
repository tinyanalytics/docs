...
Screen views are the mobile equivalent of pageviews — a screen name becomes the path you see in the [Pages](/pages) report:
...
| `event(name, props?, ctx?)` | Track a [custom event](/custom-events). |
...
- **No Core Web Vitals** — the [Performance](/performance) report is web-only.
