---
title: "React Native Analytics Integration"
sidebarTitle: "React Native"
description: "Track React Native screen views, custom events, errors, and identified users with the cookieless @tinyanalytics/react-native SDK."
icon: "react"
noindex: true
---
...
<Steps>
  <Step title="Request a mobile site">
    The dashboard's **Add website** dialog creates web sites only — mobile-app site creation
    is not currently self-serve. Ask the TinyAnalytics team to create the mobile site for you;
    existing mobile sites keep reporting and remain editable as before.
    {/* TODO: verify against product — mobile-app creation was dropped from the Add-website dialog (product 36bf8052, 2026-09-07); the API still accepts `type: "mobile"` (session-gated), so a self-serve path may return. */}
  </Step>
...
Screen views are the mobile equivalent of pageviews — a screen name becomes the path you see in the [Pages](/page-analytics) report:
...
Native apps sit behind carrier NAT and have no browser User-Agent, so the web fingerprint would collapse many devices into one. Instead the SDK mints a persistent **install ID** and sends it as `anonymousId`; the server derives a stable, site-scoped user ID from it. `identify()` layers your own product-user ID on top and backfills recent events — exactly as on the web. Without `identify()`, there are no cookies, no device fingerprinting, and no personal data at rest; once you identify users, the IDs and traits you send are personal data you're the controller for. See [how cookieless identity works](/resources/cookieless-analytics-identity).
...
| `event(name, props?, ctx?)` | Track a [custom event](/custom-event-tracking). |
...
- **No Core Web Vitals** — the [Performance](/core-web-vitals-analytics) report is web-only.
