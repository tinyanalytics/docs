...
| **Users** | The number of distinct visitors. A visitor is counted by their identified ID if you've called [identify](/identify-users), otherwise by a [cookieless fingerprint](/resources/cookieless-identity). One signed-in person on a phone and a laptop counts as one user. |
...
| **AI traffic (AI sessions)** | The number of sessions that *entered* from an [AI assistant](/ai-traffic) — a visitor who clicked through from ChatGPT, Perplexity, Gemini, Claude, Copilot, and similar. Each session is counted once, by the assistant it arrived from. |
...
<Columns cols={2}>
  <Card title="Data dictionary" icon="book" href="/data-dictionary">
