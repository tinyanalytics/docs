...
description: "How core anonymous analytics estimates visitors without cookies or a stored browser ID, using a one-way server-derived identifier while never storing raw IP addresses."
icon: "fingerprint"
---

Core anonymous analytics estimates visitors without cookies or a stored browser ID. tinyanalytics derives a one-way identifier on the server from the visitor's IP address and browser, then stores the result instead of the raw IP address.
...
<Note>
  `identify()` persists the user ID you provide in browser storage. Feature flags, groups, surveys, and browser opt-out also store purpose-specific state. Core anonymous analytics does not store an analytics ID in the browser. See [Privacy & GDPR](/resources/privacy).
</Note>
