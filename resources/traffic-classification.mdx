...
**Sibling subdomains are the exception.** A hop from `app.example.com` to `blog.example.com` is still classified as a **Referral**, because neither domain contains the other. To fold those into your direct traffic, add your root domain to **Referrer Exclusions** — see [exclude traffic](/exclude-traffic#how-do-i-stop-a-referrer-from-showing-up-as-a-referral).
...
<Columns cols={2}>
  <Card title="AI traffic" icon="microchip" href="/ai-traffic">
    Visits that arrive from AI assistants.
  </Card>
  <Card title="Shortlinks" icon="link" href="/shortlinks">
