---
title: "Privacy-Friendly Analytics and GDPR"
sidebarTitle: "Privacy and GDPR"
description: "Evaluate privacy-friendly analytics with no core analytics cookies, no raw IP storage, server-derived identity, and explicit optional browser state."
icon: "user-shield"
---
...
- **No raw IP addresses stored.** A visitor's IP is used only in memory to derive a [hashed identifier](/resources/cookieless-analytics-identity) and an approximate location, then it's gone — it isn't kept with your data.
...
| --- | --- | --- |
| [`identify()`](/user-identification-analytics) | The user ID you provide | Keep a signed-in user identified across pages and visits |
| [Feature flags](/feature-flag-analytics) | A random, site-specific visitor ID | Keep a visitor in the same variation |
| [Group analytics](/b2b-account-analytics) | The group associations you provide | Retain account or workspace context |
| [Surveys](/product-feedback-surveys) | Eligibility, impression, and completion markers | Respect wait periods and avoid repeat prompts |
| [Opt-out](/exclude-traffic-from-analytics#how-do-i-hide-my-own-traffic) | A disabled marker | Remember that analytics should stay off in that browser |
...
- Use a **non-identifying ID** (like an internal user ID or a hash) with [`identify()`](/user-identification-analytics) rather than an email address.
- Keep raw personal information out of [custom event](/custom-event-tracking) properties and traits unless you have a reason and a basis to store it.
- If you [connect Stripe](/stripe-revenue-analytics), leave **Email fallback** off unless you need it and have a basis for processing customer emails. The email is not stored in the analytics event store, but matching it is still personal-data processing.
...
<Columns cols={2}>
  ...
  <Card title="TinyAnalytics vs the alternatives" icon="scale-balanced" href="/resources/tinyanalytics-vs-google-analytics-plausible-umami">
