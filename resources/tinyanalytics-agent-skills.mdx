...
| `tinyanalytics-events` | Instruments [custom events](/custom-event-tracking) from the browser and server side, [revenue](/revenue-analytics) on events, [user identity and traits](/user-identification-analytics), and [B2B account tracking](/b2b-account-analytics) |
