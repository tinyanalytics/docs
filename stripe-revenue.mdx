...
Connect Stripe to bring server-verified payments into your site's Revenue report. tinyanalytics receives new charges, refunds, and lost disputes through a Stripe webhook, then attributes them to sessions or identified users when matching analytics IDs are available.
...
Connecting Stripe also adds `stripe.com` to the site's [referrer exclusions](/exclude-traffic#how-do-i-stop-a-referrer-from-showing-up-as-a-referral), so a return from Stripe does not start a new acquisition session.
...
| **Client events** | revenue sent with [`event()`](/revenue#choose-a-revenue-source) or the Tracking API |
...
Stripe revenue appears in the Revenue report and in `scoped_stripe_revenue` queries. [Goals](/goals), [funnels](/funnels), [experiments](/experiments), and [revenue alerts](/alerts) currently use client revenue events only.
...
<Columns cols={2}>
  ...
  <Card title="Site settings" icon="gear" href="/site-settings">
