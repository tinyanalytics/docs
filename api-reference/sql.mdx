...
`POST /api/sites/{id}/query` runs a **read-only SQL query** against your site's analytics data and returns the rows with typed column metadata — the programmatic form of the [SQL query builder](/sql-query-builder). You write a `SELECT` against two virtual tables that are already scoped to your site.
