...
`POST /api/sites/{id}/query` runs a **read-only SQL query** against your site's analytics data and returns rows with typed column metadata — the programmatic form of the [SQL query builder](/sql-query-builder). You write a `SELECT` against virtual tables that are already scoped to your site.
...
See [query Stripe revenue](/sql-query-builder#how-do-i-query-stripe-revenue) for the table schema and currency caveats.
