...
description: "Connect Claude Code, Cursor, VS Code, or Codex to TinyAnalytics over MCP with a scoped API key and 64 tools for analytics and setup."
icon: "plug"
noindex: true
...
The complete server catalog contains **64 tools** in ten domains. A full-access key receives the
**56 default tools**. A restricted key receives only the tools allowed by its selected resources;
...
Google Search Console data is available in the connected site's [Search Console
report](/google-search-console-analytics), but the MCP catalog has no GSC tool.
`get_query_schema` describes the SQL views available to `run_query`; it does not
list every TinyAnalytics integration or tell you whether your site has connected GSC.

Search queries, clicks, impressions, CTR, and average position are outside those SQL
views. Use the Search Console report for those metrics. TinyAnalytics does not expose
index coverage or URL inspection results; use Google Search Console for indexation checks.
...
<AccordionGroup>
  <Accordion title="What is the TinyAnalytics MCP server?">
    ...
    64-tool catalog over the Model Context Protocol. A full-access key receives
    56 default tools; a restricted key receives the subset allowed by its
  ...
  <Accordion title="Can an agent delete my analytics data?">
    ...
    **Users: Write** can expose user erasure. A full-access key receives 56
