...
| **Events and people** | Custom event names and properties, the raw event log, the data dictionary, user and session lists, user-trait reads (`get_user_traits`: keys, values of one key, or users matching key=value), trait writes (`merge_user_traits` to set only the keys you send — the recommended write; `update_user_traits` to replace all traits wholesale), and server-side identification |
