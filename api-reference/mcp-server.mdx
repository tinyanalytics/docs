...
description: "Connect Claude Code, Cursor, VS Code, or Codex to TinyAnalytics over MCP with a scoped API key and 66 tools for analytics and setup."
icon: "plug"
noindex: true
...
The complete server catalog contains **66 tools** in eleven domains. A full-access key receives the
**58 default tools**. A restricted key receives only the tools allowed by its selected resources;
...
| --- | --- |
| **Traffic questions** | Overview totals, metric time series (or one sessions series per value of a dimension — `get_timeseries` with `dimension`, for example `ai_assistant` to see which assistant is rising), breakdowns by 18 dimensions (with an optional `first_seen` date per value), live visitors, landing pages, acquisition, journeys, retention, and revenue |
| **Google Search Console** | Connection status and search performance by query, page, country, device, or date, with prior-period comparisons |
...
| **Onboarding guides** | Create a checklist or product tour (`create_guide` — it starts as a draft), launch, stop, or restore it (`update_guide` lifecycle fields), list guides, and read started/completed/dismissed counts plus the per-step funnel (`get_guide_stats`, `get_guide_step_stats`) |
...
Use `get_gsc_status` to check the connection and selected Google property, then
...
report](/google-search-console-analytics) first. A disconnected or unfinished
...
| Argument | Meaning |
| --- | --- |
| `site_id` | Required numeric site ID from `list_sites`. |
| `start_date`, `end_date` | Required inclusive `YYYY-MM-DD` dates in Pacific time (`America/Los_Angeles`). Maximum 366 calendar days per call; split longer ranges. |
| `dimension` | Required: `query`, `page`, `country`, `device`, or `date`. |
| `compare` | Boolean, default `false`. `true` adds the previous equal-length period to each row as `previous`; an unmatched row has `previous: null`. |
| `limit` | Maximum rows returned, 1–100, default 100. |

Each row includes `name`, `clicks`, `impressions`, `ctr` (a fraction from 0 to 1),
...
Three tools that return whole rows accept an optional `fields` argument — `list_sessions`,
...
<Steps>
  ...
  <Step title="Keep the key out of your repository">
    If the configuration file is committed, read the key from your environment instead of pasting it. In Claude Code, a project `.mcp.json` supports `${VAR}`. Export `TINYANALYTICS_API_KEY` before starting Claude Code:
    ...
    ```json .mcp.json
    {
      "mcpServers": {
        "TinyAnalytics": {
          "type": "http",
          "url": "https://dash.tinyanalytics.io/api/mcp",
          "headers": { "Authorization": "Bearer ${TINYANALYTICS_API_KEY}" }
        }
      }
    }
    ```
...
The `time_zone` argument takes an [IANA time zone name](https://www.iana.org/time-zones). The MCP server validates it before calling the API: an unknown zone is rejected immediately with examples (`UTC`, `America/New_York`, `Europe/Istanbul`) and the database link, not a bare API error.
