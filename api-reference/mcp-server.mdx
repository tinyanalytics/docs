...
description: "Connect Claude Code, Cursor, VS Code, or Codex to tinyanalytics over MCP with a scoped API key and 54 tools for analytics and setup."
icon: "plug"
noindex: true
...
The complete server catalog contains **54 tools** in nine domains. A full-access key receives the
**47 default tools**. A restricted key receives only the tools allowed by its selected resources;
...
- **Write scopes can include permanent actions.** A restricted key with an explicit matching write
  ...
  Annotations, or Users. Do not grant those write scopes to an agent that only needs reads. The 47
...
| A tool returns `403` | The key lacks the required resource scope, its owner can't access the site, or the action needs a higher organization role. Review the key's permissions and ask the agent to list your sites. |
...
<AccordionGroup>
  <Accordion title="What is the tinyanalytics MCP server?">
    ...
    54-tool catalog over the Model Context Protocol. A full-access key receives
    47 default tools; a restricted key receives the subset allowed by its
  ...
  <Accordion title="Can an agent delete my analytics data?">
    ...
    **Users: Write** can expose user erasure. A full-access key receives 47
