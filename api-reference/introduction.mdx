...
The tinyanalytics HTTP API lets you **send events from a server** and **read your analytics programmatically**, alongside the browser [tracking script](/tracking-script). Every endpoint lives under one base URL:
...
| --- | --- |
| **Sending events from a browser** | None — the `data-site-id` in the [snippet](/tracking-script) identifies the site. |
...
<AccordionGroup>
  <Accordion title="Do I need an API key to send events from the browser?">
    No. Browser ingestion needs no credential — the `data-site-id` in the [tracking script](/tracking-script) identifies the site. You need an API key to send events from a server, to read analytics, or to manage sites.
  </Accordion>
