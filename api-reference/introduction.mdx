...
<AccordionGroup>
  <Accordion title="Do I need an API key to send events from the browser?">
    No. Browser ingestion needs no credential — the `data-site-id` in the [tracking script](/guides/tracking-script) identifies the site. You need an API key to send events from a server, to read analytics, or to manage sites.
  </Accordion>
