...
The main view lists each custom event by name with its total count, most frequent first. These are the events you send with [`event()`](/custom-events) — `signup`, `purchase`, `newsletter_signup`, and so on. Pageviews aren't listed here; they have their own reports.
...
- **Turn events into goals and funnels.** Once an event is firing, use it in [Goals](/goals) and [Funnels](/funnels).

## Frequently asked questions

<AccordionGroup>
  <Accordion title="Why don't pageviews show up in the Events explorer?">
    The tinyanalytics Events explorer lists only the custom events you send with [`event()`](/custom-events), such as `signup` or `purchase`. Pageviews are excluded because they have their own reports — see [Pages](/pages) and the [Dashboard overview](/dashboard-overview).
  </Accordion>
  ...
  <Accordion title="Why can't I find an older event with the log's search box?">
    Search matches only the events already loaded into the log, which starts at the newest 100 and grows as you scroll. It isn't a search over your full history. Scroll down to load more events, or narrow your date range and [filters](/filters) to bring the event you want into range.
  </Accordion>
