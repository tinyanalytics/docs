...
description: "See your site's real-user Core Web Vitals in tinyanalytics — LCP, CLS, INP, FCP, and TTFB at the p50, p75, p90, and p99 percentiles, broken down by page, country, and device."
icon: "gauge-high"
---
...
With it on, tinyanalytics sends **one performance event per page load** carrying the five vitals. Until data arrives, the Performance report shows a notice explaining how to enable collection. See [Configure the tracking script](/script-configuration#collect-core-web-vitals) for the full attribute reference.
...
Turn on [period comparison](/compare) to add a baseline and a change indicator to each metric card. Performance comparison is aware that **lower is better** for Core Web Vitals: a rising LCP is a regression, so its change indicator turns red with an up arrow, not green.
...
<AccordionGroup>
  ...
  <Accordion title="Why does a rising LCP show as a red increase instead of green?">
    Because lower is better for Core Web Vitals. tinyanalytics period comparison knows this, so a rising LCP is treated as a regression and its change indicator turns red with an up arrow rather than green. Turn on [period comparison](/compare) to add a baseline to each metric card.
  </Accordion>
...
<Columns cols={2}>
  <Card title="Configure the tracking script" icon="sliders" href="/script-configuration#collect-core-web-vitals">
  ...
  <Card title="Compare periods" icon="arrow-right-arrow-left" href="/compare">
