---
title: "Verify Your TinyAnalytics Installation"
sidebarTitle: "Verify your setup"
...
<Steps>
  ...
  <Step title="Watch Realtime">
    In TinyAnalytics, open your site's **Realtime** report. Your visit appears within a few seconds. On a brand-new site, the setup panel re-checks automatically every few seconds and is replaced by your dashboard once the first live pageview lands — no reload needed.
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="I don't see any data at all" icon="circle-xmark">
    ...
    - **The tracking script isn't on the live page.** View the page source of your published site and confirm the `<script … src="…script.js?siteId=…">` tag is in the `<head>`. Drafts and previews don't count. (A script optimizer that strips `data-*` attributes can't remove the site ID — it rides on the URL.)
    - **The site ID is wrong.** The `?siteId=` parameter on the script URL (or the legacy `data-site-id` attribute) must match the site you're viewing in the dashboard. An invalid ID makes the script do nothing.
  ...
  <Accordion title="Requests are blocked by an ad blocker" icon="shield">
    ...
    To collect data reliably from all visitors, serve TinyAnalytics through a [first-party proxy](/install-tinyanalytics-tracking-script#bypass-ad-blockers) on your own domain, so requests are first-party and aren't blocked.
  </Accordion>
  <Accordion title="Single-page app pageviews are wrong" icon="route">
    SPA route tracking is on by default. If you see **too few** pageviews, confirm `data-track-spa` isn't set to `"false"`. If you see **double** pageviews, you're likely sending manual pageviews on top of automatic ones — either remove your manual `pageview()` calls or set `data-auto-pageview="false"`. See [configure the script](/configure-tinyanalytics-tracking-script#track-single-page-apps).
  </Accordion>
  <Accordion title="I imported historical data and the setup panel is still showing" icon="clock-rotate-left">
    An import (GA4, Plausible, Simple Analytics, Umami) is not an installation. The setup panel stays up — with a note that imported history was found — until the live tracking snippet sends its first pageview. Your imported data is already on the dashboard behind the panel; install the snippet (or fix it with the steps above) and the panel clears on its own within seconds of the first live visit.
  </Accordion>
  <Accordion title="My custom events don't appear" icon="hand-pointer">
    - The event name can't be empty — an empty name is rejected.
    - The call must run after the script loads. If you call `event()` very early in the page, guard it with `if (window.tinyanalytics)`. See [track custom events](/custom-event-tracking#when-can-i-call-the-event-api).
  </Accordion>
  <Accordion title="A specific path isn't being tracked" icon="filter">
    Check your tracking script for `data-skip-paths` — a path matching a skip pattern is never tracked. If a path shows up under a pattern like `/orders/*` instead of its real value, that's `data-mask-paths` working as intended to hide sensitive URLs. See [hide sensitive paths](/configure-tinyanalytics-tracking-script#hide-sensitive-paths-with-skip-and-mask).
  </Accordion>
</AccordionGroup>
