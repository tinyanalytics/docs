...
<AccordionGroup>
  ...
  <Accordion title="My custom events don't appear" icon="hand-pointer">
    ...
    - The call must run after the script loads. If you call `event()` very early in the page, guard it with `if (window.tinyanalytics)`. See [track custom events](/custom-event-tracking#when-is-windowtinyanalytics-available).
  </Accordion>
