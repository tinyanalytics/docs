...
- a **custom event** — a conversion happens when a visitor fires a matching [event](/guides/custom-events), or
- an **autocaptured interaction** — a conversion happens when a visitor triggers a matching [autocapture](/guides/autocapture) event (an outbound link, button click, form submission, or copy).
...
Yes. You can build a goal on any of four [autocapture](/guides/autocapture) event types without writing a custom event:
...
<Note>
  **`input_change` isn't a goal target.** Individual field changes are captured for the [Events explorer](/guides/events) but are too high-volume to build a conversion on, so they can't be used as a goal. Autocapture goals also can't aggregate [revenue](/guides/revenue) — attach revenue to a [custom event](/guides/custom-events) instead.
</Note>
