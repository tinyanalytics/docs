---
title: "Bots & AI crawlers report"
description: "See automated traffic separately from humans in tinyanalytics — bot requests, their share of traffic, and which AI crawlers (GPTBot, ClaudeBot, PerplexityBot, and more) are reading your site."
icon: "spider"
---
...
The report opens with four cards summarizing automated traffic for the current window and filters:

| Card | What it means |
| --- | --- |
| **Bot requests** | Total automated requests detected. |
| **Bot share** | Bot requests as a percentage of all traffic (bots + humans). |
| **UA pattern** | Bots identified by a known user-agent pattern. |
| **Bot ASN** | Bots identified by a known hosting/crawler network. |

## How does tinyanalytics detect bots?

tinyanalytics flags a request as a bot in two ways: a **known user-agent pattern** (the signature a crawler announces itself with) and a **known network** (the ASN a crawler operates from, which catches bots that hide their user-agent). A request flagged either way is counted here instead of in your human analytics. Detected bots are given synthetic sessions and are kept entirely out of your session counts and live visitor counter, so automated traffic can never inflate your real numbers.
...
<Info>
  **Crawling is not citation.** This report shows that an AI crawler *read* a page — the step that comes before you're cited in an AI answer. It cannot tell you whether a crawl produced a citation, and no user-agent or referrer can: that needs the AI platforms' own data. To see the *other* end of the funnel — humans who clicked through from an AI answer — use the [AI traffic report](/guides/ai-traffic).
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="Does a crawl mean an AI assistant cited my page?">
    No. The Bots & AI crawlers report shows only that an AI crawler read a page — the step that comes before a citation. No user-agent or referrer can tell you whether a crawl produced a citation. To see humans who clicked through from an AI answer, use the [AI traffic report](/guides/ai-traffic).
  </Accordion>
</AccordionGroup>
