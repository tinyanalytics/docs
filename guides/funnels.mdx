...
- an **autocaptured interaction** — an outbound link, button click, form submission, or copy (see [autocapture](/guides/autocapture)).
...
<AccordionGroup>
  <Accordion title="What can I use as a funnel step in tinyanalytics?">
    A funnel step is a page (matched by its path), a custom event (matched by its name), or an [autocaptured](/guides/autocapture) interaction — an outbound link, button click, form submission, or copy. You add steps in the order visitors should follow them, name the funnel, and save it. You can create as many funnels as you need for different flows.
  </Accordion>
  <Accordion title="Can a funnel step be a button click or outbound link?">
    Yes. A step can be an [autocapture](/guides/autocapture) interaction — an outbound link, button click, form submission, or copy — with no custom event to write. Give the step a value pattern (with `*` and `**` wildcards) to match a specific interaction, or enter `*` to match any event of that type.
  </Accordion>
