...
The second argument is optional metadata: any extra properties you want stored alongside the error, the same way [custom event](/custom-events) properties work. Manual tracking works whenever the script is loaded, and pairs well with automatic capture for the errors your code already handles gracefully.
...
**Click an error to open its details** and read every individual occurrence: the exact time, the visitor's page, browser, OS, device, and country, the person (linked to their [user profile](/users)), the full message, and the stack trace with a clickable `file:line:column` reference.

The report honors your dashboard's [date range](/dashboard-overview), [filters](/filters), and [segments](/segments) — so you can narrow to errors on one page, one browser, or one release.
