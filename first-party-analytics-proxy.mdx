...
<Steps>
  ...
  <Step title="Check the ingestion route">
    Send an intentionally incomplete payload:

    ```bash
    curl -si -X POST https://yourdomain.com/stats/api/track \
      -H "Content-Type: application/json" \
      -d '{}'
    ```

    Expect a TinyAnalytics JSON validation response with status `400`. A `404`, `405`, or HTML
    response means the proxy did not preserve the path or method.
  </Step>

  <Step title="Observe a real event">
    Open your deployed site with browser developer tools, filter the Network panel by `/stats`, and
    navigate once. `script.js` should return `200`, and `/stats/api/track` should return `204`.
    If you use heatmaps, surveys, flags, or Web Vitals, their optional requests should use the same
    prefix.
  </Step>
