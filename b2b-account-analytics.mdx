...
<AccordionGroup>
  ...
  <Accordion title="Why does an account's page show more sessions than the accounts list?">
    Because the two answer different questions. The **accounts list** is scoped to the dashboard date range, so it shows who was active in that window. An individual **account's page** always shows its full history, labelled **All time** above the numbers — the account's lifetime totals, its traits, and the people who have ever used it. An account with 1 session today can correctly show 58 sessions all time.
  </Accordion>
