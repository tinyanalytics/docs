...
<Check>
  The install is working when your own visit appears in the dashboard within a few seconds. If it doesn't, see [verify your setup](/verify-your-setup).
</Check>
...
You can turn any of these off, and turn on optional captures like Web Vitals, with [`data-*` attributes](/script-configuration).
...
- **Frameworks** (Next.js, React, Vue, Svelte, and others) — add the tag to your root layout or document head component so it loads on every route. Single-page route changes are tracked automatically ([SPA routing](/script-configuration#track-single-page-apps) is on by default).
