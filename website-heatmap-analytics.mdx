...
Click capture is **off by default**. Enable it per site under **Site Settings → Apps →
...
Open **Engagement → Heatmaps** and choose a mode, then pick a page from the list
(searchable, busiest first). Scroll lists pages with engagement data even if click capture
is off or has no data. Switching back to Clicks restores the captured-click page list:
...
- [`GET /api/sites/:id/heatmap/pages`](/api-reference/heatmaps/get-heatmap-pages) lists pages with
  click data by default. Add `source=scroll` to list pages with existing engagement data
  instead; `clicks` is zero for these rows. Both sources return up to 100 pages ranked by
  sessions, with the top page's map payloads included.
