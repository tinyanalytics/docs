---
title: "Exclude Internal and Unwanted Traffic"
sidebarTitle: "Exclude traffic"
description: "Exclude internal or unwanted traffic by IP, country, path, hostname, user agent, or bot status before it enters your analytics reports."
...
<Warning>
  **Referrer exclusions don't drop traffic.** Unlike the five lists above, the visit is still recorded in full — only its referrer is cleared and its [channel](/resources/analytics-channel-attribution) becomes Direct. This is the one item on the Exclusions tab that keeps your data.
</Warning>

Enter the narrowest host that represents the return hop, such as `checkout.stripe.com` or `login.example.com`. A few rules worth knowing:
...
- **UTM tags still win.** A visit from an excluded referrer that carries [UTM parameters](/resources/analytics-channel-attribution#tag-your-own-links) keeps its UTM-derived channel — only the stored referrer is cleared.

<Warning>
  Do not exclude a broad search domain such as `google.com`. Because a bare
  domain also covers its subdomains, that would erase real Google search
  attribution. Technical sign-in hops such as `accounts.google.com` are already
  handled automatically.
</Warning>

The list holds up to **100 entries** of up to **256 characters** each. It applies to incoming tracked traffic, not to [imported historical data](/migrate-analytics-data-to-tinyanalytics) or [short links](/trackable-short-links).
...
<Tabs>
  <Tab title="One device (your browser)">
    Set a `localStorage` opt-out key so that browser stops sending anything. This is the quickest way to exclude yourself. See [opt out of tracking](/configure-tinyanalytics-tracking-script#opt-out-of-tracking).
  </Tab>
...
<AccordionGroup>
  <Accordion title="What's the difference between excluding and filtering traffic?">
    An exclusion drops data at collection, before it's ever stored — the traffic never reaches your reports and can't be recovered. A [filter](/analytics-filters) only changes what a report *shows*; the data is still collected. Use an exclusion for traffic you never want to measure, and a filter to focus a report on traffic you did.
  </Accordion>
  <Accordion title="Can I recover traffic that TinyAnalytics excluded?">
    No. Excluded traffic is dropped at collection and never stored, so there's nothing to restore. If you might want the data later, use a [filter](/analytics-filters) instead — a filter changes what a report shows without dropping anything.
  </Accordion>
