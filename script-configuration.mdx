...
tinyanalytics then records uncaught exceptions and unhandled promise rejections as `error` events, grouped by message in the **Errors** report. Benign `ResizeObserver` warnings and cross-origin third-party script errors are filtered out automatically. See [error tracking](/errors) for the report and the manual `trackError()` call.
