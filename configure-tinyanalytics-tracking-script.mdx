...
The one exception is the site ID, which rides on the script URL as `?siteId=` rather than as an attribute. Script optimizers (Rocket Loader–class tools and some CMS minifiers) strip `data-*` attributes, which would silently disable tracking — a URL parameter can't be stripped. The `data-site-id` attribute form keeps working indefinitely, but every snippet the dashboard issues uses the URL form.

```html
<script defer src="https://dash.tinyanalytics.io/script.js?siteId=123"
  data-web-vitals
...
<Note>
  Optimizer resilience covers the site ID only. An aggressive optimizer can still strip the *other* `data-*` options on this page (`data-mask-paths`, `data-skip-paths`, `data-api-host`, capture toggles, and so on) — if you rely on those and use such an optimizer, exclude the TinyAnalytics tag from optimization.
</Note>
...
| --- | --- | --- |
| `?siteId=` on the `src` URL | *(required)* | Your site's numeric ID. Without a valid ID, the script does nothing. The legacy `data-site-id` attribute form remains supported indefinitely; if both are present and differ, `?siteId=` wins and the script logs a console warning naming both values. |
